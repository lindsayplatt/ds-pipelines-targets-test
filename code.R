...
# loads data
download_data <- function(local_file, sb_file) {
  item_file_download('5d925066e4b0c4f70d0d0599', names = sb_file, 
                     destinations = local_file, overwrite_file = TRUE)
  return(local_file)
}

process_data <- function(data_file) {
  readr::read_csv(data_file, col_types = 'iccd') %>%
...
# generates one plot
make_plot <- function(plot_file, eval_data) {
  png(file = plot_file, width = 8, height = 10, res = 200, units = 'in')
...
# generates two comma-delimited tables
write_summary_data_out <- function(out_file, eval_data) {
  readr::write_csv(eval_data, path = out_file)
}

# generates a diagnostic log file
save_diagnostics <- function(out_file, eval_data) {
