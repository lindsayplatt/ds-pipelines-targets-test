map_timeseries <- function(site_info, plot_files, out_file) {
  # libraries: leaflet, leafpop, htmlwidgets

  # prepare data
  map_data <- tibble(filename = plot_files) %>%
    extract(col='filename', into='state_cd', regex='3_visualize/tmp/timeseries_([[:alpha:]]{2})\\.png', remove=FALSE) %>%
